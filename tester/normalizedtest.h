...
bool executeFunction(bool(*func)(Q...) , std::vector<T>& params,std::index_sequence<Seq...>){
...
template<typename T>
bool NormalizedTest<T>::execute(std::vector<T>& params){
    return _func(params);
}


template<typename T>
size_t NormalizedTest<T>::getArgumentCount() const{
    return _paramCount;
}
