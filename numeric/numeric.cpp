...
bool Numeric::operator>(const Numeric &num) const {
 ...
 size_t Numeric::size() const {
  return _buffer.size;
}

 void Numeric::randomize(bool isSigned) {
  _ioModule.randomize(_buffer,isSigned);
}

Numeric Numeric::createRandom(size_t size, bool isSigned) {
  Numeric random(size);
  random.randomize(isSigned);
  return random;
}
 ...
 std::string Numeric::toBin() {
  return _ioModule.getBin(_buffer,_arthModule);
}
...
bool Numeric::isSigned(){
  return _arthModule.isSigned(_buffer);
}
